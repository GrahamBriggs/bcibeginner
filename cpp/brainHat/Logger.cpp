...
void Logger::Start()
{
	int port = OpenServerSocket(MULTICAST_LOGPORT, MULTICAST_GROUPADDRESS);
...
void Logger::RunFunction()
{
	...
	while (ThreadRunning)
	{
		...
		list<LoggerLog*> commands;
		{
			...
			{
				LoggerLog* nextCommand = CommandQueue.front();
		...
		for (nextItem = commands.begin(); nextItem != commands.end(); ++nextItem)
		{
			...
			if (log->Level >= LogDisplayLevel)
			{
				...
				{
					WriteMulticastString(format("log?log=%s\n", log->SerializeAsJson().c_str()));
				}
